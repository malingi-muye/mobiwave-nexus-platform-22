...
### 2. Admin Profiles (System Administrators)
- **Table**: `admin_profiles` (linked to `auth.users`)
- **Authentication**: Supabase Auth + admin profile verification
...
3. Verifies user has entry in `admin_profiles` table
4. Checks role is `admin` or `super_admin`
...
1. **"Access denied" for admin login**
   - Ensure user has entry in `admin_profiles` table
   - Verify role is `admin` or `super_admin`
   - Check user is confirmed in `auth.users`
...
3. **User redirected to wrong dashboard**
   - Check role assignment in appropriate profile table
   - Verify `fetchUserRole` function checks both `admin_profiles` and `profiles`
