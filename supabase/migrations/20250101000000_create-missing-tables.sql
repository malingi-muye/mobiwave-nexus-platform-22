...
INSERT INTO campaigns (user_id, name, description, campaign_type, status, recipient_count, sent_count, delivered_count, created_at)
SELECT 
  p.id,
  'Sample Campaign ' || generate_series,
  'This is a sample campaign for testing purposes',
  (ARRAY['sms', 'email', 'whatsapp'])[floor(random() * 3 + 1)],
  (ARRAY['sent', 'sending', 'scheduled', 'draft'])[floor(random() * 4 + 1)],
  floor(random() * 1000 + 100)::integer,
  floor(random() * 900 + 50)::integer,
  floor(random() * 850 + 40)::integer,
